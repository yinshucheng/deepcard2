...
# 统一开发控制台（首选，主目录）：参数化启动 + status/logs/stop
./scripts/dev.sh             # api + web 前台（最常用）
./scripts/dev.sh api         # 只起后端
./scripts/dev.sh --bg        # 全后台，日志写 /tmp/deepcard-dev/
./scripts/dev.sh all --db    # 先 db:push 再起
./scripts/dev.sh status      # 看端口/进程/health/worktree 状态
./scripts/dev.sh logs api    # 跟踪后端日志
./scripts/dev.sh stop        # 停掉主目录 api+web（不碰 worktree）
./scripts/dev.sh --help      # 全部参数

# 也可直接用底层 pnpm 脚本（dev.sh 内部就是调它们）：
pnpm dev                     # 同时起 api(:8787) + web(:5173)
pnpm dev:api / pnpm dev:web  # 单独起后端 / 前端
...
> 主目录用 `scripts/dev.sh`（端口 8787/5173）；多特性并行用 `scripts/wt.sh`（每 worktree 独立端口对），两者互不干扰。
